...
!pip install -q python-docx langchain-core langchain-openai langchain-community langchain-text-splitters chromadb crewai crewai-tools
...
# 1.2 Clone your GitHub repo into /content
!rm -rf MIE1624_GP
!git clone https://github.com/bjorn-uoft/MIE1624_GP.git

import os
print("Repo contents:", os.listdir("/content/MIE1624_GP"))
...
from pathlib import Path
BASE_DIR = Path("/content/MIE1624_GP")
...
for _, row in df_cifar.iterrows():
    pillar = row["pillar"]
    text = f"""CIFAR pillar analysis – {pillar}
...
for _, row in df_comp.iterrows():
    country = row["country"]
    text = f"""Country competitiveness & investments – {country}:
...
for entry in entries:
    ...
    text = f"""AI strategy summary – {country}:
...
chatbot_agent = Agent(
    ...
    goal=(
        ...
        "based on our MIE1624 project (Parts 1–4)."
    ),
    backstory=(
        "You are the official chatbot for Group 10’s MIE1624 project. "
...
st.set_page_config(
    page_title="MIE1624 – Canada AI Strategy Chatbot (Group 10)",
    page_icon="🤖",
    layout="wide",
)

st.title("🤖 MIE1624 – Canada’s AI Strategy Chatbot (Group 10)")
st.markdown(
    """
...
@st.cache_resource(show_spinner="Loading chatbot and project documents…")
def init_chatbot():
...
# Input field
user_q = st.chat_input("Ask about Canada’s AI strategy and our project findings…")
if user_q:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking with our project documents…"):
            try:
